package com.pig4cloud.pig.pig.controller;
...
import com.pig4cloud.pig.pig.entity.PatientDeviceEntity;
import com.pig4cloud.pig.pig.service.PatientDeviceService;
...
public class PatientDeviceController {
    ...
    @GetMapping("/page" )
    @PreAuthorize("@pms.hasPermission('pig_patientDevice_view')" )
    public R getPatientDevicePage(@ParameterObject Page page, @ParameterObject PatientDeviceEntity patientDevice) {
    ...
    @SysLog("新增" )
    @PostMapping
    @PreAuthorize("@pms.hasPermission('pig_patientDevice_add')" )
    ...
    @SysLog("修改" )
    @PutMapping
    @PreAuthorize("@pms.hasPermission('pig_patientDevice_edit')" )
    ...
    @SysLog("通过id删除" )
    @DeleteMapping
    @PreAuthorize("@pms.hasPermission('pig_patientDevice_del')" )
    ...
    @ResponseExcel
    @GetMapping("/export")
    @PreAuthorize("@pms.hasPermission('pig_patientDevice_export')" )
    public List<PatientDeviceEntity> export(PatientDeviceEntity patientDevice,Long[] ids) {
        return patientDeviceService.list(Wrappers.lambdaQuery(patientDevice).in(ArrayUtil.isNotEmpty(ids), PatientDeviceEntity::getPddId, ids));
    }
}